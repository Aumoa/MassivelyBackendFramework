...
namespace cpp_backend
{
    ...
    namespace
    {
        ...
        {
            return kind == packet_kind::request || kind == packet_kind::response || kind == packet_kind::notify ||
        ...
        {
            return kind == master_node_kind::gateway || kind == master_node_kind::dedicated ||
                   kind == master_node_kind::master_admin || kind == master_node_kind::backend;
        }
        ...
        void write_backend_packet_manifest_entry(packet_writer& writer, const backend_packet_manifest_entry& entry)
        {
        ...
        void write_backend_packet_manifest(packet_writer& writer, const backend_packet_manifest& manifest)
        {
        ...
        void require_control_frame(const packet_frame& frame, std::uint16_t packet_id, std::uint16_t version)
        {
            if (frame.header.kind != packet_kind::control || frame.header.flags != 0 ||
                frame.header.packet_id != packet_id || frame.header.version != version ||
        ...
        socket_owner listen_tcp(const gateway_listener_endpoint& endpoint, std::uint16_t& bound_port)
        {
            ...
            int enabled = 1;
            setsockopt(listener.get(), SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&enabled),
                       sizeof(enabled));
...
#else
                int result =
                    static_cast<int>(::send(socket, bytes.data() + sent, static_cast<std::size_t>(chunk_size), 0));
#endif
                if (result <= 0)
                {
    ...
    packet_error::packet_error(const std::string& message) : std::runtime_error(message) {}
    ...
    direct_connect_code_validation_response sidecar_direct_connect_code_validator::validate(
        const std::string& code, const std::string& gateway_node_id, const std::string& gateway_master_connection_id)
    {
        ...
        auto request_frame =
            make_frame(packet_kind::control, sidecar_pid_direct_connect_validation_request,
                       sidecar_control_schema_version, encode_direct_connect_code_validation_request(request));
        ...
        require_control_frame(response_frame, sidecar_pid_direct_connect_validation_response,
    ...
    sidecar_control_ack send_sidecar_ack_request(const sidecar_control_endpoint& endpoint,
                                                 std::uint16_t request_packet_id, std::uint16_t ack_packet_id,
                                                 std::vector<std::uint8_t> payload, const guid_bytes& request_id)
    {
        auto socket = connect_tcp(endpoint);
        auto request_frame =
            make_frame(packet_kind::control, request_packet_id, sidecar_control_schema_version, std::move(payload));
        write_socket_frame(socket.get(), request_frame);

        auto ack_frame = read_socket_frame(socket.get(), sidecar_control_max_payload_length);
        require_control_frame(ack_frame, ack_packet_id, sidecar_control_schema_version);
    ...
    sidecar_control_client::sidecar_control_client(sidecar_control_endpoint endpoint) : m_endpoint(std::move(endpoint))
    {
    }

    sidecar_control_ack sidecar_control_client::update_endpoint_state(bool ready, std::string detail)
    {
        ...
        return send_sidecar_ack_request(m_endpoint, sidecar_pid_endpoint_state_update, sidecar_pid_endpoint_state_ack,
                                        encode_sidecar_endpoint_state_update(update), request_id);
    }

    sidecar_control_ack sidecar_control_client::update_shutdown_state(bool shutting_down, std::string reason)
    {
        ...
        auto request_id = update.request_id;
        return send_sidecar_ack_request(m_endpoint, sidecar_pid_shutdown_state_update, sidecar_pid_shutdown_state_ack,
    ...
    {
    public:
        impl(gateway_listener_endpoint endpoint, node_auth_challenge challenge,
             direct_connect_code_validator& validator)
            : m_listener(listen_tcp(endpoint, m_port)), m_challenge(std::move(challenge)), m_validator(&validator)
        {
        }
    ...
    gateway_direct_listener::gateway_direct_listener(gateway_listener_endpoint endpoint, node_auth_challenge challenge,
                                                     direct_connect_code_validator& validator)
        : m_impl(std::make_unique<impl>(std::move(endpoint), std::move(challenge), validator))
    {
    }
    ...
    void packet_writer::write_string(const std::string& value)
    {
        ...
        write_bytes(std::span(reinterpret_cast<const std::uint8_t*>(value.data()), value.size()));
    }
    ...
    packet_reader::packet_reader(std::span<const std::uint8_t> bytes) : m_bytes(bytes) {}
    ...
    {
        ensure_available(2);
        auto value = static_cast<std::uint16_t>((static_cast<std::uint16_t>(m_bytes[m_position]) << 8) |
    ...
    {
        ensure_available(4);
        std::uint32_t value = (static_cast<std::uint32_t>(m_bytes[m_position]) << 24) |
    ...
    std::array<std::uint8_t, packet_header::size> encode_header(const packet_header& header)
    {
        ...
        bytes[0] = static_cast<std::uint8_t>((static_cast<std::uint8_t>(header.kind) << kind_shift) |
    ...
    packet_header decode_header(std::span<const std::uint8_t> bytes, std::uint32_t max_payload_length)
    {
        ...
        auto packet_id = static_cast<std::uint16_t>((static_cast<std::uint16_t>(bytes[1]) << 8) |
                                                    static_cast<std::uint16_t>(bytes[2]));
        auto version = static_cast<std::uint16_t>((static_cast<std::uint16_t>(bytes[3]) << 8) |
                                                  static_cast<std::uint16_t>(bytes[4]));
        auto payload_length = (static_cast<std::uint32_t>(bytes[5]) << 16) |
        ...
        packet_header header{
            kind, flags, packet_id, version, payload_length,
        };
        validate_header(header);
        return header;
    }
    ...
    packet_frame make_frame(packet_kind kind, std::uint16_t packet_id, std::uint16_t version,
                            std::vector<std::uint8_t> payload, std::uint8_t flags)
    {
        ...
        packet_header header{
            kind, flags, packet_id, version, static_cast<std::uint32_t>(payload.size()),
        };
        validate_header(header);
        return packet_frame{header, std::move(payload)};
    }
    ...
    std::vector<std::uint8_t> encode_sidecar_manifest_snapshot_response(const sidecar_manifest_snapshot_response& value)
    {
    ...
    {
        return make_frame(packet_kind::control, master_pid_node_auth_challenge, master_control_schema_version,
                                                                      ...
                                                                      const std::string& backend_connection_id)
    {
        ...
        auto validation = validator.validate(code.code, hello.node_id, hello.master_connection_id);
        if (!validation.success)
        {
            throw packet_error(validation.error_message.empty() ? "Direct connect code validation failed."
        ...
        return gateway_direct_handshake_result{
            ...
            make_frame(packet_kind::control, master_pid_node_accepted, master_control_schema_version,
                       encode_node_accepted(accepted)),
        };
    }

    trusted_gateway_session::trusted_gateway_session(std::string gateway_node_id,
    ...
    void trusted_gateway_session::write_channel_data(gateway_frame_writer& writer, std::uint32_t channel_id,
                                                     packet_kind routed_kind, std::uint16_t routed_packet_id,
                                                     ...
                                                     std::vector<std::uint8_t> routed_payload)
    {
        ...
        auto frame = make_frame(routed_kind, pid_gate_backend_channel_data, gateway_backend_channel_version,
                                encode_gateway_backend_channel_data_envelope(envelope));
        std::lock_guard lock(m_write_sync);
        writer.write(std::move(frame));
    }

    void trusted_gateway_session::write_channel_close(gateway_frame_writer& writer, std::uint32_t channel_id,
                                                      const std::string& reason)
    {
        ...
        auto frame = make_frame(packet_kind::notify, pid_gate_backend_channel_close, gateway_backend_channel_version,
