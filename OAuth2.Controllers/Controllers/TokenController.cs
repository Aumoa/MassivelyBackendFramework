...
[ApiController]
[Route("api/v1/token")]
public class TokenController(IAuthorizationCodes authorizationCodes, IAccesses accesses, IJwt jwt, IAccounts accounts, IAccountClaims accountClaims, IClientClaims clientClaims, IClientUserGroups groups, IAccountRoles roles, ITokenIssuer tokenIssuer, IOptions<HostOptions> hostOptions, IApiKeys apiKeys, IClients clients, ILogger<TokenController> logger) : ControllerBase
{
    [HttpPost]
    ...
    private async ValueTask<IActionResult> HandleRefreshTokenAsync(TokenRequest request, CancellationToken cancellationToken)
    {
        ...
        var rolesClaim = await roles.GetAccountRolesAsync(newAccess.Value.Sub, cancellationToken);
