...
public static class RemoteDebugClientAuthenticator
{
    public static byte[] ComputeProof(
        RemoteDebugAuthChallenge challenge,
        RemoteDebugHandshakeHello hello,
        string sharedSecret)
    {
        if (challenge == null)
        {
        ...
        WriteString(payload, hello.ClientId);
        WriteString(payload, hello.DisplayName);
        WriteString(payload, hello.ClientVersion);
        WriteString(payload, hello.UnityVersion);
        WriteUInt32(payload, (uint)hello.Capabilities);
        ...
        string sharedSecret)
    {
        ...
        if (!string.Equals(hello.ClientId, proof.ClientId, StringComparison.Ordinal))
        {
            return false;
        }
