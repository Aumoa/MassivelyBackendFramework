...
public sealed class RemoteDebugGatewayClientOptions
{
    ...
    internal void Validate()
    {
        ...
        {
            throw new InvalidOperationException("RemoteDebug Gateway host is required.");
        }

        if (Port <= 0 || Port > 65535)
        {
            throw new InvalidOperationException("RemoteDebug Gateway port must be between 1 and 65535.");
        }

        if (RequestTimeoutMilliseconds <= 0)
        {
            throw new InvalidOperationException("RemoteDebug Gateway request timeout must be greater than zero.");
        }

        if (string.IsNullOrWhiteSpace(BackendKind))
        {
            throw new InvalidOperationException("RemoteDebug Backend kind is required.");
        }
